...
| 앱 재실행 | 유효한 Keychain token으로 현재 계정을 복원함 | 미실행 |
| 만료 token | Keychain을 비우고 로그인이 필요하다고 표시함 | 미실행 |
| 서버 401 | Keychain을 비우고 로그인이 필요하다고 표시함 | 미실행 |
| 로그아웃 | Keychain token을 삭제하고 로그인 화면으로 돌아감 | 미실행 |
